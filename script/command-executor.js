    ...
    .then(function (accessKey) {
      ...
      if (willUpdateTtl) {
        ...
        if (willUpdateName) {
          ...
          logMessage +=
            'changed the expiration date of the "' +
            command.oldName +
            '" access key to ' +
            expirationDate;
        }
...
function accessKeyRemove(command) {
  return exports.confirm().then(function (wasConfirmed) {
    if (wasConfirmed) {
  ...
  if (normalizedOs === "ios") {
    ...
    return Q.reject(
      new Error(
        '"' +
          command.os +
          '" is an unsupported OS. Available options are "ios", "android", and "windows".'
      )
    );
  }
  var platform;
  ...
  if (normalizedPlatform === "react-native") {
    ...
    return Q.reject(
      new Error(
        '"' +
          command.platform +
          '" is an unsupported platform. Available options are "react-native" and "cordova".'
      )
    );
  }
  return exports.sdk
    .addApp(command.appName, os, platform, false)
    .then(function (app) {
      exports.log(
    ...
    .then(function (wasConfirmed) {
      if (wasConfirmed) {
        return exports.sdk.removeApp(command.appName).then(function () {
          exports.log(
            'Successfully removed the "' + command.appName + '" app.'
          );
        });
      }
...
function deleteConnectionInfoCache(printMessage) {
  ...
  try {
    ...
    if (printMessage) {
      exports.log(
        "Successfully logged-out. The session file located at " +
            ...
            return exports.sdk
              ...
              .then(function (metrics) {
                if (metrics[deployment.package.label]) {
                  var totalActive = getTotalActiveFromDeploymentMetrics(
                    metrics
                  );
...
function execute(command) {
  ...
  return Q(null).then(function () {
    switch (command.type) {
      ...
      // Must be logged in
      default:
        if (!!exports.sdk) break; // Used by unit tests to skip authentication
        if (!connectionInfo) {
          throw new Error(
    ...
    printTable(headers, function (dataSource) {
      deploymentHistory.forEach(function (packageObject) {
        ...
        if (packageObject.releaseMethod === "Promote") {
          releaseSource =
            "Promoted " +
            packageObject.originalLabel +
            ' from "' +
            packageObject.originalDeployment +
            '"';
          ...
          releaseSource =
            "Rolled back " + lastLabel + " to " + packageObject.originalLabel;
        }
        if (releaseSource) {
          releaseTime +=
        ...
        if (packageObject.isDisabled) {
          row = row.map(function (cellContents) {
            return applyChalkSkippingLineBreaks(cellContents, chalk.dim);
          });
        }
        dataSource.push(row);
      });
...
function getReactNativeProjectAppVersion(command, projectName) {
  ...
  if (command.platform === "ios") {
    ...
    if (resolvedPlistFile) {
      ...
      if (!resolvedPlistFile) {
        throw new Error(
          "Unable to find either of the following plist files in order to infer your app's binary version: \"" +
            knownLocations.join('", "') +
    ...
    if (parsedPlist && parsedPlist.CFBundleShortVersionString) {
      if (isValidVersion(parsedPlist.CFBundleShortVersionString)) {
        ...
        throw new Error(
          'The "CFBundleShortVersionString" key in the "' +
            resolvedPlistFile +
            '" file needs to specify a valid semver string, containing both a major and minor version (e.g. 1.3.2, 1.1).'
        );
      }
    } else {
      throw new Error(
        'The "CFBundleShortVersionString" key doesn\'t exist within the "' +
          resolvedPlistFile +
          '" file.'
      );
    }
    ...
    return g2js
      ...
      .catch(function () {
        throw new Error(
          'Unable to parse the "' +
            buildGradlePath +
            '" file. Please ensure it is a well-formed Gradle file.'
        );
      })
      .then(function (buildGradle) {
        ...
        if (
          ...
          throw new Error(
            'The "' +
              buildGradlePath +
              '" file doesn\'t specify a value for the "android.defaultConfig.versionName" property.'
          );
        }
        if (typeof versionName !== "string") {
          throw new Error(
            'The "android.defaultConfig.versionName" property value in "' +
              buildGradlePath +
              '" is not a valid string. If this is expected, consider using the --targetBinaryVersion option to specify the value manually.'
          );
        }
        ...
        if (isValidVersion(appVersion)) {
          ...
          throw new Error(
            'The "android.defaultConfig.versionName" property in the "' +
              buildGradlePath +
              '" file needs to specify a valid semver string, containing both a major and minor version (e.g. 1.3.2, 1.1).'
          );
        }
          ...
          if (fileExists(propertiesFile)) {
            ...
            try {
              ...
              throw new Error(
                'Unable to parse "' +
                  propertiesFile +
                  '". Please ensure it is a well-formed properties file.'
              );
            }
          }
        }
    ...
    try {
      ...
      throw new Error(
        'Unable to find or read "' +
          appxManifestFileName +
          '" in the "' +
          path.join("windows", projectName) +
          '" folder.'
      );
    }
    return parseXml(appxManifestContents)
      ...
      .then(function (parsedAppxManifest) {
        try {
          return parsedAppxManifest.Package.Identity[0]["$"].Version.match(
            /^\d+\.\d+\.\d+/
          )[0];
        } catch (e) {
          throw new Error(
            'Unable to parse the package version from the "' +
              path.join(appxManifestContainingFolder, appxManifestFileName) +
              '" file.'
          );
        }
...
function promote(command) {
  ...
  return exports.sdk
    ...
    .catch(function (err) {
      return releaseErrorHandler(err, command);
    });
}
...
exports.release = function (command) {
  ...
  return Q(null).then(function () {
    ...
    var releaseHooksPromise = index_1.default.reduce(function (
      accumulatedPromise,
      hook
    ) {
  ...
  return validateDeployment(command.appName, command.deploymentName)
    .then(function () {
      ...
      if (platform === "ios") {
        ...
        var outputFolderVer7 = path.join(
          platformFolder,
          "app",
          "src",
          "main",
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            "Unable to " +
              cordovaCommand +
              " project. Please ensure that either the Cordova or PhoneGap CLI is installed."
          );
        }
      }
      exports.log(
      ...
      try {
        ...
        throw new Error(
          "Unable to " +
            cordovaCommand +
            ' project. Please ensure that the CWD represents a Cordova project and that the "' +
            platform +
            '" platform was added by running "' +
            cordovaCLI +
            " platform add " +
            platform +
            '".'
      ...
      try {
        var configString = fs.readFileSync(
      ...
      return configPromise.catch(function (err) {
        throw new Error(
...
exports.releaseReact = function (command) {
  ...
  function runHermesEmitBinaryCommand(
    bundleName,
    outputFolder,
    sourcemapOutput
  ) {
  ...
  return validateDeployment(command.appName, command.deploymentName)
    .then(function () {
      ...
      try {
        var projectPackageJson = require(path.join(
          process.cwd(),
          "package.json"
        ));
    ...
    .then(function () {
      return deleteFolder(os.tmpdir() + "/react-*");
    })
    .then(function () {
      if (platform === "android") {
        return exports
      ...
      return exports.runReactNativeBundleCommand(
        bundleName,
        command.development || false,
        entryFile,
        outputFolder,
...
exports.runReactNativeBundleCommand = function (
  bundleName,
  development,
  entryFile,
  outputFolder,
...
function serializeConnectionInfo(
  ...
  exports.log(
    "\r\nSuccessfully logged-in. Your session file was written to " +
      chalk.cyan(configFilePath) +
      ". You can run the " +
      chalk.cyan("code-push logout") +
      " command at any time to delete this file and terminate your session.\r\n"
  );
}
function sessionList(command) {
  throwForInvalidOutputFormat(command.format);
  return exports.sdk.getSessions().then(function (sessions) {
...
function sessionRemove(command) {
  if (os.hostname() === command.machineName) {
    ...
    return exports.confirm().then(function (wasConfirmed) {
      if (wasConfirmed) {
        return exports.sdk.removeSession(command.machineName).then(function () {
          exports.log(
            'Successfully removed the login session for "' +
              command.machineName +
              '".'
          );
        });
...
function getSdk(accessKey, headers, customServerUrl, proxy) {
   ...
   */
  Object.getOwnPropertyNames(AccountManager.prototype).forEach(function (
    functionName
  ) {
